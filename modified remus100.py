...
class remus100:
    ...
    def __init__(
        ...
        self.rho = 1020                 # density of water (kg/m^3)
        ...
        self.nMax = 3931                # max propeller revolution (rpm)    
        ...
        self.MA = np.diag([ m*k1, m*k2, m*k2, k_prime*Ix, k_prime*Iy, k_prime*Iy ])
        #using fossen calculations instead of the MA matrix used in the matlab code.
        ...
        # For our ROV, 4 horizontal facing thrusters are used to control
        ...
        #leave autopilot part in, decide on values next time
        # Heading autopilot
    ...
    def dynamics(self, eta, nu, u_actual, u_control, sampleTime):
        ...
        #D_prop = 0.076   # propeller diameter corresponding to 5.5 inches
        ...
        # Ja_max = 0.944 * 4 / (0.076 * 3931/60) = 0.7583
        #Ja_max = 0.6632
        Ja_max = 0.7583
        
        # Single-screw propeller with 3 blades and blade-area ratio = 0.718. for our ROV is 0.537
        # Coffes. are computed using the Matlab MSS toolbox:     
        # >> [KT_0, KQ_0] = wageningen(0,1,0.718,3)
        # [KT_0, KQ_0] = wageningen(0,1,0.537,3)
        #KT_0 = 0.4566
        #KQ_0 = 0.0700
        KT_0 = 0.4148
        KQ_0 = 0.0617
        ...
        if n_rps > 0:   # forward thrust
            ...
            #root 2 multiple is to account for the 45 angle and 2 thrusters    
            X_prop = math.sqrt(2)*self.rho * pow(D_prop,4) * ( 
            ...
            X_prop = math.sqrt(2) * self.rho * pow(D_prop,4) * KT_0 * abs(n_rps) * n_rps 
            #Y_prop = math.sqrt(2) * self.rho * pow(D_prop,4) * KT_0 * abs(n_rps) * n_rps 
            #Z_prop = 4* self.rho * pow(D_prop,4) * KT_0 * abs(n_rps) * n_rps 
        ...
        tau_liftdrag = forceLiftDrag(self.width,self.S,self.CD_0,alpha,U_r)
        tau_crossflow = crossFlowDrag(self.L,self.width,self.height,nu_r)
        ...
        if ny_rps > 0:
            ...
            Y_prop = math.sqrt(2)*self.rho * pow(D_prop,4) * ( 
                KT_0 * abs(ny_rps) * ny_rps + (KT_max-KT_0)/Ja_max * 
                (Va/D_prop) * abs(ny_rps) )
        
        
        else:
            Y_prop = math.sqrt(2) * self.rho * pow(D_prop,4) * KT_0 * abs(ny_rps) * ny_rps 
            
        
        Ky_prop = self.rho * pow(D_prop,5) * (
                    KQ_0 * abs(ny_rps) * ny_rps + (KQ_max-KQ_0)/Ja_max * 
                    (Va/D_prop) * abs(ny_rps) )
            
        if nz_rps > 0:    
            Z_prop = 4 * self.rho * pow(D_prop,4) * ( 
                KT_0 * abs(nz_rps) * nz_rps + (KT_max-KT_0)/Ja_max * 
                (Va/D_prop) * abs(nz_rps) )    


        else:
            Z_prop = 4* self.rho * pow(D_prop,4) * KT_0 * abs(nz_rps) * nz_rps 
            
        Kz_prop = self.rho * pow(D_prop,5) * (
            KQ_0 * abs(nz_rps) * nz_rps + (KQ_max-KQ_0)/Ja_max * 
            (Va/D_prop) * abs(nz_rps) )
        """
        """
        ...
        # assume the stern plane and rudder can be modeled as the props that dont generate moments
        ...
        #ny_dot = (n_y - n_yy) / self.T_n
        delta_s_dot = (delta_s_c - delta_s) / self.T_delta
        #nz_dot = (n_z - n_zz) / self.T_n
        n_dot = (n_c - n) / self.T_n 

        # Forward Euler integration [k+1]
        nu += sampleTime * nu_dot
        delta_r += sampleTime * delta_r_dot
        #n_yy += sampleTime * ny_dot
        delta_s += sampleTime * delta_s_dot
        #n_zz += sampleTime * nz_dot
        n += sampleTime * n_dot
        
        #u_actual = np.array([ delta_r, delta_s, n ], float)
        u_actual = np.array([ delta_r, delta_s, n ], float)
        print("dynamics u_actual", u_actual)
        #u_actaul accepts degs in the first 2 array positions, cannot use rpm
    ...
    def stepInput(self, t):
        ...
        n = 3931                     # propeller revolution (rpm)
        ...
        u_control = np.array([ 0, 0, n], float)
        print("stepinput:", u_control)
        return u_control
    ...
    def depthHeadingAutopilot(self, eta, nu, sampleTime):
        ...
        print("depthautopilot: ", u_control)
        return u_control
